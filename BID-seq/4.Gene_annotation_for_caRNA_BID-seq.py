...
### usage: cat caRNA-shctr-BID-seq.pU_site.txt | python ~/Genome/tools_custom/BID-seq/4.Gene_annotation_for_caRNA_BID-seq.py 1.txt > caRNA-shctr-BID-seq.pU_site.annotated.txt
    ...
    def call_relative_pos(self, pos):
        ...
        if self.gene_direction == '-':
            ...
            if gene_struc[1] <= pos <= gene_struc[0]:
                ...
                return "NA"
class repeats():
    def __init__(self, Chr, direction, txStart_site, txEnd_site, repeats_name):
        self.Chr = Chr
        self.direction = direction
        self.txStart_site = txStart_site
        self.txEnd_site = txEnd_site
        self.repeats_name = repeats_name
    def call_relative_pos(self, pos):
        if self.direction == '+':
            a = '0'
            b = '1'
        ...
        if gene_direction == "+":
            ...
            if refseq_gene.Chr not in gene_annotation_dict_r.keys():
                ...
                gene_annotation_dict_r[refseq_gene.Chr].append(
                    [refseq_gene, refseq_gene.txStart_site, refseq_gene.txEnd_site])
with open('/home/yutaozhao/Genome/hg38-UCSC-Repeats', 'r') as f:
    for line in f:
        repeats_info = list(item.strip() for item in line.split('\t'))
        Chr = repeats_info[0]
        direction = repeats_info[3]
        repeats_name = repeats_info[4] + '|' + repeats_info[5] + '|' + repeats_info[6]
        if direction == "+":
            txStart_site = int(repeats_info[1])
            txEnd_site = int(repeats_info[2])
            repeat = repeats(Chr, direction, txStart_site, txEnd_site, repeats_name)
            if Chr not in repeats_dict_f.keys():
                repeats_dict_f[Chr] = [[repeat, txStart_site, txEnd_site]]
            else:
                repeats_dict_f[Chr].append([repeat, txStart_site, txEnd_site])
        if direction == "-":
            txStart_site = int(repeats_info[2])
            txEnd_site = int(repeats_info[1])
            repeat = repeats(Chr, direction, txStart_site, txEnd_site, repeats_name)
            if Chr not in repeats_dict_r.keys():
                repeats_dict_r[Chr] = [[repeat, txStart_site, txEnd_site]]
            else:
                repeats_dict_r[Chr].append([repeat, txStart_site, txEnd_site])
...
for line in sys.stdin:
    ...
    if direction == "+" and chr_info in repeats_dict_f.keys():
        for repeat in repeats_dict_f[chr_info]:
            if repeat[2] >= pos >= repeat[1]:
                ...
                if motif[3] == 'U':
                    ...
                    if motif[4] == 'U':
                        ...
                        motif = motif + '|' + motif4
                print(
                    f'{chr_info}\t{pos}\t{direction}\t.\t{repeat[0].repeats_name}\t.\t.\t.\t.\t{motif}\t{del_ratio}\t{reads_distri}')
                save_unidentified = False
                break
        if save_unidentified == False:
            continue
    if direction == "-" and chr_info in repeats_dict_r.keys():
        for repeat in repeats_dict_r[chr_info]:
            if repeat[1] >= pos >= repeat[2]:
